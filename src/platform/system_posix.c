...
bool PlatformExecutableDirectory(wchar_t* output, uint32_t capacity)
{
    ...
    path[length - 1] = '\0';
    return PlatformUtf8ToWide(path, (uint32_t)(length - 1),
        output, capacity, NULL);
}

static bool EncodeUtf8Scalar(uint32_t scalar, char* output,
                             uint32_t capacity, uint32_t* offset)
{
    uint32_t required = scalar <= 0x7fU ? 1U
        : scalar <= 0x7ffU ? 2U
                        ...
                        uint32_t capacity, uint32_t* outLength)
{
    ...
    output[offset] = '\0';
    if (outLength != NULL) *outLength = offset;
    return true;
}

static bool DecodeUtf8Scalar(const uint8_t* input, uint32_t length,
                             uint32_t* offset, uint32_t* scalar)
{
    ...
    {
        count = 1; value = first & 0x1fU; minimum = 0x80U;
    }
    else if ((first & 0xf0U) == 0xe0U)
    {
        count = 2; value = first & 0x0fU; minimum = 0x800U;
    }
    else if ((first & 0xf8U) == 0xf0U)
    {
        count = 3; value = first & 0x07U; minimum = 0x10000U;
    }
    else return false;
                        ...
                        uint32_t capacity, uint32_t* outLength)
{
    ...
    {
        uint32_t scalar;
        if (!DecodeUtf8Scalar((const uint8_t*)input, length,
                &inputOffset, &scalar)
    ...
    if (outLength != NULL) *outLength = outputOffset;
    return true;
}

static bool WidePathToUtf8(
    const wchar_t* path, char output[LAIUE_PLATFORM_PATH_CAPACITY * 4U])
{
    if (!PlatformWideToUtf8(path, output,
            LAIUE_PLATFORM_PATH_CAPACITY * 4U, NULL))
        return false;
...
bool PlatformCreateDirectory(const wchar_t* path)
{
    ...
    if (!WidePathToUtf8(path, nativePath)) return false;
    if (mkdir(nativePath, 0755) == 0) return true;
    return errno == EEXIST;
}
...
bool PlatformPathExists(const wchar_t* path)
{
    char nativePath[LAIUE_PLATFORM_PATH_CAPACITY * 4U];
    struct stat status;
    return WidePathToUtf8(path, nativePath)
        && lstat(nativePath, &status) == 0;
}
...
bool PlatformMoveReplace(const wchar_t* source, const wchar_t* destination)
{
    ...
    return WidePathToUtf8(source, nativeSource)
        && WidePathToUtf8(destination, nativeDestination)
                            ...
                            uint8_t** outBytes, uint64_t* outSize)
{
    ...
    *outSize = 0;
    ...
    struct stat status;
    bool valid = fstat(file, &status) == 0 && S_ISREG(status.st_mode)
        && status.st_size >= 0 && (uint64_t)status.st_size <= maximumBytes;
    uint8_t* bytes = valid
        ? PlatformAllocate(status.st_size == 0 ? 1U : (size_t)status.st_size, false)
        : NULL;
    uint64_t completed = 0;
    while (valid && completed < (uint64_t)status.st_size)
    {
        ssize_t result = read(file, bytes + completed,
            (size_t)((uint64_t)status.st_size - completed));
        if (result < 0 && errno == EINTR) continue;
        if (result <= 0) valid = false;
        else completed += (uint64_t)result;
    }
    close(file);
    if (!valid)
    {
...
static bool WriteAll(int file, const void* bytes, uint64_t size)
{
    ...
    {
        ssize_t result = write(file, source + completed,
    ...
    PlatformDirectoryIterator* iterator, PlatformDirectoryEntry* entry)
{
    ...
    for (;;)
    {
        ...
        if (!PlatformUtf8ToWide(result->d_name, (uint32_t)nameLength,
                entry->name, LAIUE_PLATFORM_PATH_CAPACITY, NULL))
            continue;
...
void PlatformDirectoryClose(PlatformDirectoryIterator* iterator)
{
    ...
    PosixDirectoryIterator* state = (PosixDirectoryIterator*)iterator;
    if (state->directory != NULL) closedir(state->directory);
    free(state->path);
    memset(iterator, 0, sizeof(*iterator));
}
...
bool PlatformSha256(const void* bytes, uint64_t size, uint8_t output[32])
{
    ...
    bool succeeded = EVP_DigestInit_ex(context, EVP_sha256(), NULL) == 1;
    const uint8_t* input = bytes;
    uint64_t offset = 0;
    while (succeeded && offset < size)
    {
        size_t part = size - offset > SIZE_MAX
            ? SIZE_MAX : (size_t)(size - offset);
        succeeded = EVP_DigestUpdate(context, input + offset, part) == 1;
        offset += part;
    }
    unsigned int outputLength = 0;
    if (succeeded)
        succeeded = EVP_DigestFinal_ex(context, output, &outputLength) == 1
            && outputLength == 32U;
...
{
    char nativePath[LAIUE_PLATFORM_PATH_CAPACITY * 4U];
    if (!WidePathToUtf8(path, nativePath)) return NULL;
