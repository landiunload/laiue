...
// Immediate-mode интерфейс: виджеты каждый
...
// Единая палитра интерфейса и виджетов.
...
LAIUE_UI_API bool UiBegin(UiContext* ui, int32_t width, int32_t height,
...
LAIUE_UI_API void UiSetClip(
    UiContext* ui, float x, float y, float width, float height);
LAIUE_UI_API void UiClearClip(UiContext* ui);

LAIUE_UI_API void UiRelease(UiContext* ui);

// Примитивы (координаты в пикселях окна).
LAIUE_UI_API void UiRect(
    UiContext* ui, float x, float y, float width, float height,
    float cornerRadius, uint32_t color);
LAIUE_UI_API void UiImage(
    UiContext* ui, float x, float y, float width, float height,
    float u0, float v0, float u1, float v1, uint32_t color);
LAIUE_UI_API void UiText(
    UiContext* ui, float x, float lineTopY, uint32_t color,
    const wchar_t* text);
LAIUE_UI_API void UiTextCentered(
    UiContext* ui, float centerX, float lineTopY,
    uint32_t color, const wchar_t* text);
LAIUE_UI_API float UiTextWidth(
    const UiContext* ui, const wchar_t* text);

// Общие композиционные элементы интерфейса используют один стиль,
// поэтому тени, панели и строки значений не дублируются по модулям.
LAIUE_UI_API void UiPanel(
    UiContext* ui, float x, float y, float width, float height);
LAIUE_UI_API float UiLabelValueRow(
    UiContext* ui, float x, float width, float y,
    const wchar_t* label, const wchar_t* value, float bottomGap);

// Анимация значения к цели (для наведения и переключателей).
LAIUE_UI_API float UiAnimate(
    UiContext* ui, uint32_t id, bool towardOne);

// Виджеты: true — действие в этом кадре (нажатие/изменение значения).
LAIUE_UI_API bool UiButton(
    UiContext* ui, uint32_t id, float x, float y,
    float width, float height, const wchar_t* label);
LAIUE_UI_API bool UiSliderInt(
    UiContext* ui, uint32_t id, float x, float y,
    float width, int32_t minimum, int32_t maximum, int32_t* value);
LAIUE_UI_API bool UiToggle(
    UiContext* ui, uint32_t id, float x, float y, bool* value);
LAIUE_UI_API bool UiRadioRow(
    UiContext* ui, uint32_t id, float x, float y,
