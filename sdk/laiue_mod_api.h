 ...
 * нативного DLL/SO-мода. Исходники игры не требуются: мод не линкуется
 ...
 *
 *   LAIUE_MOD_EXPORT void LAIUE_MOD_CALL
 *   LaiueModShutdown(void);   // опционально
...
/* Экспорт и ABI entry points. Макросы одинаково работают в C и C++;
   extern "C" для C++-мода автор добавляет к определениям самостоятельно. */
#if defined(_WIN32)
#define LAIUE_MOD_EXPORT __declspec(dllexport)
#define LAIUE_MOD_CALL __cdecl
#elif defined(__GNUC__) || defined(__clang__)
#define LAIUE_MOD_EXPORT __attribute__((visibility("default")))
#define LAIUE_MOD_CALL
#else
#error Unsupported compiler for the laiue native mod ABI
#endif
...
typedef void (*LaiueBlockEditCallback)(void *user, int64_t x, int64_t y, int64_t z,
...
typedef struct LaiueModApi
{
    ...
    bool (*setBlock)(void *host, int64_t x, int64_t y, int64_t z, uint8_t block);
    ...
    void (*setFrameCallback)(void *host, LaiueFrameCallback callback, void *user);
    /* Правки блоков игроком (ломание/установка). Изменения из
       setBlock самого мода событие не порождают. */
    void (*setBlockEditCallback)(void *host, LaiueBlockEditCallback callback, void *user);
    ...
    void (*setAirJumps)(void *host, int32_t extraJumps, float impulse, bool refillOnGround);
     ...
     * причину в журнал. */
    bool (*publishInterface)(void *host, const char *name, uint32_t version,
                             void *interfacePointer);
    void *(*queryInterface)(void *host, const char *name, uint32_t minimumVersion);
    ...
    void (*setFixedTickCallback)(void *host, LaiueFixedTickCallback callback, void *user);
...
typedef int32_t(LAIUE_MOD_CALL *LaiueModInitFunction)(const LaiueModApi *api);
typedef void(LAIUE_MOD_CALL *LaiueModShutdownFunction)(void);
