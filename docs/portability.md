...
- ARM64 Debug дополнительно линкует `softintrin.lib`. MSVC разворачивает
  `_Interlocked*` только оптимизатором, и `/Oi` этого не меняет; на x86_64
  они разворачиваются всегда. Это программная реализация интринсиков от
  Microsoft, а не CRT, и проверка imports готовых DLL остаётся в силе.
