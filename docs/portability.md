...
- Файл, использующий `Interlocked*` на Windows, сам выдаёт для них
  `#pragma intrinsic` под `_M_ARM64`. `winnt.h` делает это только в ветках
  `_M_AMD64` и `_M_IX86`, поэтому на ARM64 при `/Od` компилятор оставляет
  вызовы CRT-помощников, которых при `/NODEFAULTLIB` нет; `/Oi` этого не
  меняет. У clang-cl прагма не нужна и намеренно выключена.
