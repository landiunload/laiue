...
Рядом с `laiue.exe` должен находиться каталог `texturepacks`:
...
`active.txt` содержит одно ASCII-имя, например `patrix_32x.ltp`. Первый символ
...
Версия 1 содержит только albedo. Normal/specular-карты Patrix (`_n`, `_s`),
анимация `.mcmeta` и схема CTM 6x6 пока не входят в runtime-формат.
