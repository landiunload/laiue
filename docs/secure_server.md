...
Каждый отказ старта печатает строку
`laiue dedicated server: startup failed: <причина> (exit <код>)` и лишь
затем завершает процесс. Пустой журнал при `Restart=on-failure` означает,
что процесс упал не на старте.

| Код | Причина | Что делать |
|---|---|---|
| 0 | штатная остановка по Ctrl+C или SIGTERM | — |
| 1 | не хватило памяти под конфигурацию или мир | освободить память, проверить `world_seed` |
| 2 | не хватило памяти под состояние сервера | то же |
| 3 | `server_enabled.txt` не даёт валидного набора модов | сверить имена, версии и совместимость модов |
| 4 | не собирается bundle для скачивания | проверить `.lmp`/`.lsp`/`.ltp` либо выключить `allow_content_downloads` |
| 5 | нет пригодных credentials или не поднялся secure transport | см. разделы выше; текст строки различает эти два случая |
