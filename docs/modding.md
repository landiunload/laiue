...
## Новый ABI технологий

Для новых модулей используйте `mod/module_api.h` и `mod/module_host.h`.
Официальная технология и пользовательское расширение имеют один ABI:
`LaiueModuleGetApiV1`, descriptor с `provides/requires` и lifecycle
`create/start/stop/destroy`. Модуль получает только опубликованные сервисы и
не линкуется с DLL движка. Минимальная сборочная fixture и проверка зависимостей
находятся в `tests/module_provider.c`, `tests/module_consumer.c` и
`tests/module_host_test.c`; архитектурные границы описаны в
`docs/module_architecture.md`.

Существующий `LaiueModLoadV1` ниже — compatibility adapter для уже выпущенных
паков. Он сохраняется временно и не должен использоваться как второй ABI для
новых технологий.
