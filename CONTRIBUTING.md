...
- Разбор чужих форматов живёт в общей внутренней `media_support`, а не
  копируется между движком и инструментами: PNG, GIF, JPEG, WAV и MP3
  читает одна реализация, и её же проверяют тесты. Выбор декодера по
  сигнатуре тоже её дело: `ImageInspect` с `ImageDecode` для картинок и
  `SoundInspect` с `SoundDecodeSamples` для звука — единственные места,
  где перечислен набор форматов.
- Табличные данные формата, которые не выводятся формулой (таблицы
  Хаффмана MP3, окно синтеза, границы полос), берутся из открытой
  публикации стандарта. Такие данные обязаны быть сверены между двумя
  независимыми источниками, снабжены записью о происхождении прямо в
  заголовке и проверены в тесте по свойствам, а не по контрольной сумме:
  полнота префиксного кода, покрытие набора значений, точность кратных.
  Сторонний код при этом не заимствуется — заимствуются числа стандарта,
  и зависимостей у движка по-прежнему нет. Декодеры собственные — сторонних
  зависимостей у движка по-прежнему нет. В SDK инструменты не
  устанавливаются, а их символы не входят в ABI модулей.
...
- Рантайм вправе писать в каталог содержимого только собственный кэш и
  только рядом с исходником, из которого кэш собран (`stone.png` даёт
  `stone.png.lt`). Файл, положенный человеком, не переписывается, имя
  кэша не может занять его место, запись атомарна, а её неудача не
  считается ошибкой загрузки: каталог бывает доступен только на чтение.
  Свежесть определяет отпечаток исходника внутри самого кэша — размер и
  время изменения, — а не сравнение дат двух файлов: исходник при
  проверке не читается, иначе кэш терял бы смысл. Точным такой отпечаток
  не является, и это записано там, где кэш описан.
- Свой формат ищется последним, а не первым: он запасной путь на случай,
  когда исходника нет, он повреждён или движку не по зубам. Иначе файл,
  собранный однажды, навсегда заслонил бы обновлённый рядом исходник.
  Порядок переопределяется файлом `formats.txt` рядом с `active.txt`, и
  он задаёт приоритет, а не белый список: формат, которого в нём нет,
  остаётся доступным — просто позже, чтобы забытая строка не прятала
  содержимое.
