...
//
// Контракт — в src/simulation/physics/compound_bvh.h. Тест намеренно проверяет
...
// компилирует `src/simulation/physics/compound_bvh.c` прямо в тестовый исполняемый файл
