...
static void InitializeWorld(void)
{
    ...
    Expect(VoxelRigidContactCacheInitialize(&legacyContacts, legacyContactStorage,
                                            TEST_CAPACITY, contactBytes), "legacy cache initialized");
    Expect(VoxelRigidContactCacheInitialize(&indexedContacts, indexedContactStorage,
                                            TEST_CAPACITY, contactBytes), "indexed cache initialized");
    ...
    Expect(VoxelRigidBroadphaseInitialize(&spatialIndex, indexStorage + 1u,
                                          TEST_CAPACITY, indexBytes), "unaligned index initialized");
}

static void InitializeBody(uint32_t slot, uint64_t id,
                             const VoxelRigidBodyDescription *description)
{
    ...
    Expect(VoxelRigidBodyInitialize(&legacyBodies[slot], id, description), "legacy body initialized");
    if (perturbFp) LaiueTestSetHostileFpEnvironment();
    Expect(VoxelRigidBodyInitialize(&indexedBodies[slot], id, description), "indexed body initialized");
}
...
static void StepBoth(uint32_t count, const VoxelRigidStepSettings *settings, bool cached)
{
    ...
    if (perturbFp) LaiueTestSetHostileFpEnvironment();
    bool legacyResult = cached
        ? VoxelRigidBodyStepCached(legacyBodies, count, &collision, settings, legacyScratch,
...
{
    for (uint32_t tick = 0u; tick < steps; ++tick) StepBoth(count, settings, true);
}
...
static void TestSleepingImpact(void)
{
    ...
    Expect(VoxelRigidBodyLinearVelocity(&indexedBodies[0], targetVelocity) &&
           VoxelRigidBodyLinearVelocity(&indexedBodies[1], incomingVelocity), "impact velocities");
...
static void ExpectSynchronizedProxies(uint32_t count)
{
    ...
    {
        if (indexedBodies[slot].active) ++activeCount;
    }
...
static void TestFullCapacityChurn(void)
{
    ...
    for (uint32_t slot = 0u; slot < TEST_CAPACITY; ++slot)
    {
        ...
        if (slot == 0u)
        {
            ...
            for (uint32_t axis = 0u; axis < 3u; ++axis) description.halfExtent[axis] = 128.0;
        }
    ...
    for (uint32_t axis = 0u; axis < 3u; ++axis) small.halfExtent[axis] = 0.25;
...
{
    InitializeWorld();
    Expect(VoxelRigidBroadphaseBytes(0u) == 0u &&
           VoxelRigidBroadphaseBytes(UINT32_MAX) == 0u, "invalid index capacity rejected");
    ...
    Expect(!VoxelRigidBroadphaseInitialize(&invalid, indexStorage + 1u, TEST_CAPACITY,
                                           indexBytes - 1u), "short index storage rejected");
    ...
    Expect(VoxelRigidBroadphaseInitialize(&invalid, indexedScratch, TEST_CAPACITY,
                                          (uint32_t)sizeof(indexedScratch)), "aliased index initialized");
    Expect(!VoxelRigidBodyStepIndexed(indexedBodies, 2u, &collision, &settings, indexedScratch,
                                      (uint32_t)sizeof(indexedScratch), &indexedContacts, &invalid),
...
static uint64_t Replay(bool reversed, bool hostileFp, bool resetIndexEachTick, bool cached)
{
    ...
    for (uint32_t tick = 0u; tick < REPLAY_TICKS; ++tick)
    {
        ...
        if (resetIndexEachTick) VoxelRigidBroadphaseReset(&spatialIndex);
...
LAIUE_TEST_ENTRY(RigidBroadphaseTestEntryPoint)
{
    ...
    Expect(hash == UINT64_C(0x01d0a286ff9911da), "indexed cached reference unchanged");
    ...
    Expect(Replay(false, false, false, false) == UINT64_C(0xfb1c9d5a02972c16),
